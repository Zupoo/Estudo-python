...
def dividir_listas (preco_imoveis, tamanho_imoveis, percentual_treino):
    i = percentual_treino*len(preco_imoveis)
    lista_treino_preco = []
    lista_teste_preco = []
    lista_treino_tamanho = []
    lista_teste_tamanho = []
    for n, item in enumerate(preco_imoveis):
        if n < i:
            lista_treino_preco.append(item)
        else:
            lista_teste_preco.append(item)
    for n, item in enumerate(tamanho_imoveis):
        if n < i:
            lista_treino_tamanho.append(item)
        else:
            lista_teste_tamanho.append(item)       
    return lista_treino_preco, lista_treino_tamanho, lista_teste_preco, lista_teste_tamanho

lista_treino_preco, lista_treino_tamanho, lista_teste_preco, lista_teste_tamanho = dividir_listas(precos_imoveis, tamanho_imoveis, 0.5)
print(f'Listas de treino: \n preco imoveis: {lista_treino_preco}\n tamanho imoveis:  {lista_treino_tamanho}')  
print('-'*100)
print(f'Listas de teste: \n preco imoveis: {lista_teste_preco}\n tamanho imoveis:  {lista_teste_tamanho}')  

print('-'*200)
print(len(precos_imoveis))
print(len(lista_treino_preco))
print(len(lista_treino_tamanho))
