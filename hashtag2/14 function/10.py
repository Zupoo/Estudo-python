...
def v_perdidas_estoque(vendas):
    perdidas_estoque = {}
    for item in vendas:
        if vendas[item][2] == 'Estoque em Falta':
            perdidas_estoque[item] = vendas[item]
    return perdidas_estoque        

def v_concluidas(vendas):
    concluidas = {}
    for item in vendas:
        if vendas[item][1] == 'Concluído':
            concluidas[item] = vendas[item]
    return concluidas

def vendas_total(vendas):
    soma = 0
    for item in vendas:
        soma = soma + vendas[item][0]
    return soma    



vendas_perdidas_estoque = v_perdidas_estoque(vendas) 
vendas_concluidas = v_concluidas(vendas)

total_perdidas_estoque = vendas_total(vendas_perdidas_estoque)
total_concluidas = vendas_total(vendas_concluidas)

stockout = (total_perdidas_estoque / (total_concluidas + total_perdidas_estoque))*100
print(stockout)
